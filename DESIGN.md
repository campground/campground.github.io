...
- **Hero:** daylight campsite (faceted twin peaks, treeline, still lake, one tent, a thread of smoke) in Pine, Moss and Pine Tint on Birch. Absolute from `lg`, 704px tall, object-fit cover anchored bottom; eager-loaded with `fetchpriority="high"`; it sits in the initial HTML, so it needs no head preload.
...
- **Format:** WebP at 2x layout size (panoramas under 200 KB, the flat lay under 100 KB), each with intrinsic `width`/`height`, a `srcset` of narrower cuts (`name-640.webp`, `-1024`, `-1280`, `-1600` for panoramas; `-640`, `-1024` for the flat lay, made from the PNG master with `cwebp -q 80 -m 6 -resize <w> 0`) and a `sizes` that matches the drawn width, `decoding="async"`, and `aria-hidden` (empty `alt`) when decorative. PNG masters and `.json` prompt sidecars live beside the plates for provenance and are excluded from the build. Any new plate is generated flat and faceted with hard edges; no painterly softness, blur or grain.
